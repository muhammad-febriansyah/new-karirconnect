...
{
    /** OpenAI Chat Completions endpoint. Override via Setting `ai.openai_base_url`. */
    ...
    {
        $apiKey = (string) $this->settings->get('ai.openai_api_key', '');
        if ($apiKey === '') {
            throw new RuntimeException('OpenAI API key belum diatur di pengaturan AI.');
        }

        $baseUrl = (string) ($this->settings->get('ai.openai_base_url', self::DEFAULT_BASE_URL) ?: self::DEFAULT_BASE_URL);
        $model = $options['model'] ?? (string) $this->settings->get('ai.openai_default_model', 'gpt-4o-mini');
    ...
    {
        return (string) ($this->settings->get('ai.openai_default_model', 'gpt-4o-mini') ?: 'gpt-4o-mini');
    }

    /**
     * Rough USD cost estimate per 1K tokens. Settings can override these via
     * `ai.openai_cost_input_per_1k` / `ai.openai_cost_output_per_1k` so we keep
     * accurate cost tracking even when OpenAI changes pricing.
     */
    private function estimateCost(string $model, int $promptTokens, int $completionTokens): float
    {
        $inputPer1k = (float) $this->settings->get('ai.openai_cost_input_per_1k', 0.00015);
        $outputPer1k = (float) $this->settings->get('ai.openai_cost_output_per_1k', 0.0006);
