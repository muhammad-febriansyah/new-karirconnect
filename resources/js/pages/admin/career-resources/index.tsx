    ...
    const columns = useMemo<ColumnDef<Item>[]>(
        () => [
            ...
            },
            {
                accessorKey: 'status',
                header: 'Status',
                // Scheduled articles are published rows that are not on the site yet,
                // so a two-state badge would show them as live and hide the mistake.
                cell: ({ row }) => {
                    ...
                    return (
                        <Badge
                            variant={
                                status === 'live' ? 'default' : 'secondary'
                            }
                        >
                            {status === 'live' ? 'Tayang' : 'Draft'}
                        </Badge>
                    );
                },
            },
            { accessorKey: 'views_count', header: 'Views' },
            {
                ...
                cell: ({ row }) => (
                    <span className="text-xs text-muted-foreground">
                        {row.original.reading_minutes} menit
                    </span>
                ),
            },
            {
                ...
                cell: ({ row }) => (
                    <span className="text-xs text-muted-foreground">
                        {row.original.published_at
                            ? `${row.original.status === 'scheduled' ? 'Terbit ' : ''}${formatDateTime(row.original.published_at)}`
                    ...
                    {items.length === 0 ? (
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="flex flex-col gap-2 text-sm text-muted-foreground sm:flex-row sm:items-center sm:justify-between">
                                <span>
                                    Menampilkan{' '}
                                    {table.getRowModel().rows.length} dari{' '}
                                    {items.length} data
                                </span>
